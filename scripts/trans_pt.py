...
def run():
    ...
    d_filt = {k[len(name) + 1:]: v for k, v in d.items() if
              k[:len(name)] == name}
    ...
    print("encode shape", encoder(F.adaptive_avg_pool2d(I, 256)).shape)
    traced_script_module_encoder = torch.jit.trace(encoder, F.adaptive_avg_pool2d(I, 256), check_trace=False)
    traced_script_module_encoder.save("./mobile_encoder.jit")
    optimized_scripted_module = optimize_for_mobile(traced_script_module_encoder, backend='cpu')
    optimized_scripted_module._save_for_lite_interpreter("mobile_encoder.pkl")
    input_names = ["input"]
    output_names = ["output"]
    path = "./mobile_encoder.onnx"
